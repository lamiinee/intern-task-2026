...
This structure means the model knows *what* to do (Task), *who* it's doing it for (Context), *why* it matters (Reasoning), *when it's done* (Stop Conditions), and *how to format it* (Output). The result is more reliable structured output with fewer schema violations.
